...
inline static struct sockaddr_ll init_addr(char *name)
{
    ...
    memcpy((void *) addr.sll_addr, (void *) name, sizeof(name));
...
void fmt_frame(Dev *self, Net net, Esp esp, Txp txp)
{
    ...
    memcpy((void *) self->frame, (void *) &net, sizeof(net));
    self->framelen += sizeof(net);
    
    memcpy((void *) self->frame + self->framelen, (void *) &esp, sizeof(esp));
    self->framelen += sizeof(esp);
    
    memcpy((void *) self->frame + self->framelen, (void *) &txp, sizeof(txp));
    self->framelen += sizeof(txp);
}
